...
var floot = {
    ...
    spotbtn: function spotbtn(spot, ban) {
        var con = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
        //万能轮播图
        var _index = 0;
        var timeStop = null;
        // 图片播放
    ...
    spot: function spot(li, _spot) {
        //轮播图的点点
        var banLen = li.length;
        var _html = "";
...
var fixeds = {
    ...
    fixBox: function fixBox() {
        var _top = $(window).scrollTop(); //窗口滚动的高度
        var _height = $('.fsBanner').height(); //盒子的高度
        if (_top > _height) {
            //滚动的高度大于盒子的高度添加固定
    ...
    backTopZoom: function backTopZoom() {
        var _top = $(window).scrollTop(); //窗口滚动的高度
        var _height = $('.fsBanner').height(); //盒子的高度
        // document.title = _top;
        if (_top > _height) {
